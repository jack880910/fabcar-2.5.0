 ...
 * Loaded via `webcam-register-lfd.html`
 */
...
const minScore = 0.5; // minimum score for minConfidence
...
function drawFace(canvas, result, fps) {
    ...
    ctx.fillText(`Face Feature Register for ${faceLabel}`, 10, 50);
        ...
        if (video && video.readyState >= 2) {
            if (video.paused) {
                ...
                if (detection == null || detection === undefined) {
                    ...
                    let confirmed = window.confirm('確定要登錄這個人臉特徵值嗎？');
                    if (confirmed) {
                        sendFace(video, canvas);
                    } else {
                        video.play();
                    }
                }
...
function sendFace(video, canvas) {
    let req = new XMLHttpRequest();
    let action = "/register-by-lfd";
    let method = "POST";
    ...
    req.onload = function () {
        ...
        if (this.responseText.toLowerCase() == 'success') {
            message = "人臉特徵值登錄成功";
        } else if (this.responseText.toLowerCase() == 'fail') {
            message = "人臉特徵值登錄失敗";
        }
        alert(message);
        // replay the video
...
async function main() {
    ...
    // initialize tfjs
    log(`Face feature register for ${faceLabel}`);
